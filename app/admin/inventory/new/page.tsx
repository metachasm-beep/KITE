...
export default function NewArtifactPage() {
  ...
  return (
    <div className="max-w-2xl">
      <header className={`border-b pb-6 mb-12 ${isCyberpunk ? "border-white/10" : "border-black/5"}`}>
        <h1 className={`text-4xl font-heading tracking-[-0.05em] uppercase 
          ${isCyberpunk ? "text-white" : "text-foreground text-outline"}`}>ALLOCATE_NEW_UNIT</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 font-mono">
        ...
        <div className="space-y-4">
          ...
          <HudContainer className="relative group p-0 border-none">
            {imagePreview ? (
              <div className={`relative aspect-video w-full border overflow-hidden
                ${isCyberpunk ? "border-accent/20 bg-black" : "border-black/5 bg-white"}`}>
                <img src={imagePreview} alt="Preview" className="w-full h-full object-cover opacity-60" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <button 
                    type="button" 
                    onClick={removeImage}
                    className={`p-3 border transition-all group/btn
                      ${isCyberpunk ? "bg-black/60 border-white/10 text-white" : "bg-white/90 border-black/5 text-foreground"}
              ...
              <label className={`flex flex-col items-center justify-center aspect-video w-full border border-dashed transition-all cursor-pointer group
                ${isCyberpunk ? "border-white/10 bg-white/[0.01] hover:bg-white/[0.03]" : "border-black/10 bg-black/[0.01] hover:bg-black/[0.02]"}
                hover:border-accent/40`}>
                <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                <Upload size={24} className="text-zinc-600 group-hover:text-accent transition-colors mb-4" />
                <TechnicalLabel label="UPLOAD_CORE_ASSET" className={`text-zinc-600 ${isCyberpunk ? "group-hover:text-white" : "group-hover:text-black"}`} />
                <span className="text-[8px] text-zinc-500 mt-2 uppercase tracking-tighter">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input required type="text" 
              className={`w-full border p-4 text-xs focus:outline-none focus:border-accent transition-colors
                ${isCyberpunk ? "bg-white/[0.02] border-white/10 text-white" : "bg-white border-black/10 text-foreground"}`} 
              value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="e.g. UNIT_01 // OMEGA" />
          </div>
          <div className="space-y-2">
            ...
            <input required type="text" 
              className={`w-full border p-4 text-xs focus:outline-none focus:border-accent transition-colors
                ${isCyberpunk ? "bg-white/[0.02] border-white/10 text-white" : "bg-white border-black/10 text-foreground"}`} 
              value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value})} placeholder="e.g. unit-01-omega" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input required type="text" 
              className={`w-full border p-4 text-xs focus:outline-none focus:border-accent transition-colors
                ${isCyberpunk ? "bg-white/[0.02] border-white/10 text-white" : "bg-white border-black/10 text-foreground"}`} 
              value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} placeholder="e.g. ₹2,999" />
          </div>
          <div className="space-y-2">
        ...
        <div className={`pt-6 border-t space-y-4 ${isCyberpunk ? "border-white/10" : "border-black/5"}`}>
           <h3 className="text-[10px] tracking-widest text-accent uppercase">TECHNICAL_SPECS</h3>
           {formData.specs.map((spec, i) => (
              <div key={i} className="flex gap-4">
                 <input type="text" 
                   className={`w-1/3 border p-3 text-xs focus:outline-none focus:border-accent/40
                     ${isCyberpunk ? "bg-white/[0.01] border-white/5 text-zinc-400" : "bg-white border-black/5 text-zinc-500"}`} 
                   value={spec.label} onChange={e => updateSpec(i, "label", e.target.value)} />
                 <input type="text" 
                   className={`flex-1 border p-3 text-xs focus:outline-none focus:border-accent/40
                     ${isCyberpunk ? "bg-white/[0.01] border-white/5 text-white" : "bg-white border-black/5 text-foreground"}`} 
                   value={spec.value} onChange={e => updateSpec(i, "value", e.target.value)} />
              </div>
           ))}
           <button type="button" onClick={() => setFormData({...formData, specs: [...formData.specs, {label:"", value:""}]})} 
             className={`text-[9px] tracking-widest uppercase transition-colors
               ${isCyberpunk ? "text-zinc-500 hover:text-white" : "text-zinc-400 hover:text-black"}`}>
              + ADD_SPEC_LINE
           </button>
        </div>
