      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <header className={`mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8 border-b pb-12 
          ...
          <div className="space-y-4">
            ...
            {isCyberpunk ? (
              <GlitchText className="text-5xl md:text-8xl font-mono text-[#00f5d4] cyber-glow uppercase" speed={0.4}>
                TECHNICAL SPECIFICATIONS
              </GlitchText>
            ) : (
              <h1 className="text-5xl md:text-8xl font-heading tracking-[-0.05em] uppercase text-foreground text-outline">
                TECHNICAL <br />
                <span className="text-zinc-200">SPECIFICATIONS</span>
              </h1>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section className="space-y-8">
              <div className="flex items-center gap-4">
                ...
                <h2 className={`text-xl font-heading tracking-tight uppercase
                  ${isCyberpunk ? "text-[#00f5d4] font-mono cyber-glow" : "text-foreground text-outline"}`}>MATERIAL_SCIENCE</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <HudContainer className={`p-6 ${isCyberpunk ? "bg-[#00f5d4]/5 border-[#00f5d4]/20 shadow-[0_0_20px_rgba(0,245,212,0.05)] rounded-none" : "bg-black/[0.01]"}`}>
                  <TechnicalLabel label="SUBSTRATE_01" value="HIGH-DENSITY_PLA" className="mb-4 text-accent" />
                  <p className={`text-[11px] font-mono leading-relaxed uppercase ${isCyberpunk ? "text-[#00f5d4]/60" : "text-zinc-500"}`}>
            ...
            <section className="space-y-8">
              <div className="flex items-center gap-4">
                ...
                <h2 className={`text-xl font-heading tracking-tight uppercase
                  ${isCyberpunk ? "text-[#00f5d4] font-mono cyber-glow" : "text-foreground text-outline"}`}>LOGISTICS_SYNC (FAQ)</h2>
              </div>
              <div className="space-y-4">
                {[
                  ...
                  <HudContainer key={i} className={`p-6 transition-colors ${isCyberpunk ? "bg-[#00f5d4]/5 border-[#00f5d4]/10 rounded-none hover:border-accent/40" : "border-black/5 hover:border-accent/10"}`}>
                    <h3 className={`text-[10px] font-bold mb-3 tracking-widest uppercase ${isCyberpunk ? "text-[#00f5d4] font-mono" : "text-zinc-400"}`}>{item.q}</h3>
                    <p className={`text-[11px] font-mono leading-relaxed uppercase ${isCyberpunk ? "text-[#00f5d4]/40" : "text-zinc-500"}`}>{item.a}</p>
                  </HudContainer>
                ))}
              </div>
            </section>
          ...
          <aside className="space-y-12">
            <div className="space-y-6">
              <div className={`flex items-center gap-3 border-b pb-4 ${isCyberpunk ? "border-[#00f5d4]/20" : "border-black/5"}`}>
                <ShieldCheck size={16} className={isCyberpunk ? "text-[#00f5d4]" : "text-zinc-500"} />
                <h2 className={`text-xl font-heading tracking-tight uppercase ${isCyberpunk ? "text-[#00f5d4]/60 font-mono" : "text-zinc-400"}`}>ORDER_LOOKUP</h2>
              </div>
              
              <HudContainer className={`p-8 border-accent/10 space-y-8 ${isCyberpunk ? "bg-black border-[#00f5d4]/20 rounded-none shadow-[0_0_40px_rgba(0,245,212,0.05)]" : "bg-muted/50"}`}>
                <div className="space-y-4">
                  ...
                  <input 
                    ...
                    className={`w-full border p-4 text-[10px] font-mono focus:outline-none focus:border-accent transition-colors uppercase tracking-widest
                      ${isCyberpunk ? "bg-[#00f5d4]/5 border-[#00f5d4]/20 text-[#00f5d4]" : "bg-white border-black/5 text-foreground"}`}
                  />
                  ...
                  >
                    {isCyberpunk ? (
                      <DecryptedText text="INITIALIZE_GEOMETRIC_SCAN" animateOn="hover" />
                    ) : (
                      "Initialize System Scan"
                    )}
                  </SpringButton>
                </div>
                <div className={`p-4 border border-dashed text-center ${isCyberpunk ? "border-[#00f5d4]/10" : "border-black/5"}`}>
                   <p className={`text-[8px] font-mono uppercase tracking-tighter ${isCyberpunk ? "text-[#00f5d4]/40" : "text-zinc-800"}`}>
